...
\begin{itemize}
    \item Membaca suhu lingkungan via sensor digital \textbf{BMP280} (I2C).
...
\texttt{timer\_control\_fan} & 200\,ms & Hitung PID, set PWM, push log \\
\bottomrule
\end{tabular}
\end{center}
...
\subsection{\texttt{sensor.c} --- Driver BMP280 via I2C}

BMP280 diakses melalui bus I2C. Register-register penting:
\begin{itemize}
    \item \textbf{0xD0} \texttt{ID} --- chip ID, harus 0x58.
...
BMP280 butuh pola ``tulis alamat register, lalu baca 6 byte data''. API baru menyediakan \texttt{i2c\_master\_transmit\_receive} yang melakukan ini dalam satu transaksi (efisien).
...
\begin{itemize}
    ...
    \item Jika STA gagal dalam 10\,s, \texttt{esp\_wifi\_set\_mode(WIFI\_MODE\_AP)} dan hidupkan AP \texttt{ThermalCtrl-AP}.
...
\midrule
\textbf{BMP280} & I2C, digital, $\pm$1$^{\circ}$C & \textbf{ Dipilih} \\
...
Alasan BMP280: digital (kebal noise), I2C 2-wire (hemat pin), built-in kompensasi, dan modul murah $\sim\$3$.
...
\begin{itemize}
    \item \textbf{Data plane}: BMP280 $\rightarrow$ I2C $\rightarrow$ ESP32 $\rightarrow$ PWM $\rightarrow$ L298N $\rightarrow$ Fan.
...
\midrule
GPIO21 (SDA) & \emph{default} I2C0 SDA & hardware pull-up 4.7\,k$\Omega$ \\
GPIO22 (SCL) & \emph{default} I2C0 SCL & hardware pull-up 4.7\,k$\Omega$ \\
...
\addlinespace
I2C noise dari motor & Digital I2C \emph{immune} terhadap EMI; pull-up 4.7\,k$\Omega$. \\
\addlinespace
...
\addlinespace
Li-ion battery safety & 3S BMS; monitoring tegangan via ADC. Low-battery warning. \\
\addlinespace
...
\begin{itemize}
    \item \textbf{Konstruksi}: ESP32 DevKit, BMP280 modul, L298N board, 12V DC fan, 2 push buttons, 3S Li-ion + BMS, breadboard + jumper.
...
\begin{itemize}
    \item Sensor digital I2C (BMP280) dengan kompensasi firmware.
...
thermal-mgmt/
  ...
  main/
    ...
    sensor.c / .h         # BMP280 I2C driver + compensation
    actuator.c / .h       # L298N LEDC PWM + PID + ramping
