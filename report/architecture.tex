...
\begin{itemize}
    ...
    \item Mengunggah data periodik ke \textbf{Google Sheets} via Apps Script Web App untuk monitoring jarak jauh.
...
\texttt{timer\_cloud\_upload} & 30\,s & POST sample terbaru ke Google Sheets (jika STA connected) \\
\bottomrule
\end{tabular}
\end{center}
...
% ============================================================
\subsection{\texttt{cloud.c} --- Upload ke Google Sheets}
% ============================================================

Modul \texttt{cloud} mengirim sample terbaru ke \textbf{Google Sheets} via \textbf{Apps Script Web App} setiap 30 detik (atau sesuai \texttt{CLOUD\_UPLOAD\_MS}). Pendekatan ini dipilih karena:
\begin{itemize}
    \item \textbf{Gratis} dan tanpa server middleware --- Apps Script dan Google Sheets berada di infrastruktur Google.
    \item \textbf{Real-time dashboard} --- cukup buka spreadsheet dari mana saja untuk monitoring.
    \item \textbf{History otomatis} --- tidak perlu download manual atau khawatir flash ESP32 penuh.
    \item \textbf{Alert-ready} --- Apps Script bisa di-\emph{trigger} untuk kirim email/Telegram jika nilai di luar ambang batas.
\end{itemize}

\paragraph{Alur kerja.}
\begin{enumerate}
    \item ESP32, ketika STA connected ke router dengan internet, mengirim \texttt{POST} ke URL Apps Script dengan JSON body:
    \begin{lstlisting}[caption={Contoh JSON body yang dikirim ESP32}]
{"sample":42,"temp":28.5,"fan":31,"mode":"AUTO","setpoint":30.0}
    \end{lstlisting}
    \item Apps Script menerima \texttt{doPost(e)}, parse JSON, dan \texttt{appendRow} ke spreadsheet.
    \item Apps Script mengembalikan HTTP 200 + \texttt{\{"ok":true\}}.
    \item ESP32 mencatat timestamp sukses di \texttt{last\_upload\_ms}.
\end{enumerate}

\paragraph{Kode Apps Script (server-side).}
\begin{lstlisting}[language=JavaScript,caption={Apps Script \texttt{doPost} handler}]
function doPost(e) {
  ...
  sheet.appendRow([
    new Date(),
    d.sample, d.temp, d.fan, d.mode, d.setpoint
  ]);
...
\paragraph{Verifikasi HTTPS dan sertifikat.}
ESP-IDF HTTP client mengirim ke \texttt{script.google.com} via HTTPS. Untuk capstone, \texttt{cert\_pem} dibiarkan kosong sehingga \emph{certificate verification} di-skip (akseptabel karena URL Apps Script di-hardcode di \texttt{config.h}). Untuk produksi, embed ISRG Root X1 (~1.4\,KB) di \texttt{cert\_pem}.

\paragraph{Batasan Gmail quota.}
Akun gratis punya \textbf{90 menit Apps Script execution/hari}. Dengan interval 30\,s = 2880 upload/hari $\approx$ 80 menit. Mendekati limit --- pertimbangkan naikkan interval ke 60\,s untuk safety margin, atau gunakan akun Google Workspace.

\paragraph{Status reporting.}
Endpoint \texttt{/api/status} JSON sekarang memuat blok \texttt{cloud}:
\begin{lstlisting}[caption={Tambahan field \texttt{cloud} di \texttt{/api/status}}]
"cloud":{"ok":42,"fail":1,"age_s":28}
\end{lstlisting}
Artinya: 42 upload berhasil, 1 gagal, upload terakhir 28 detik yang lalu.
...
thermal-mgmt/
  ...
  main/
    ...
    cloud.c / .h          # Google Sheets upload via Apps Script
